...
print(" ")
asaversion = input("Please enter 8.2 to any version below 8.3 or 8.3 for any other version: ")
# Ask the user for SSH port, is none is informed, use the default (22)
print(" ")
remoteport = int(input("Please inform the SSH port to connect to ASA. Just press enter if using the default port (22): ") or 22)
...
if asaversion == '8.2':
    ...
    ssh_username = input("Please inform the username to be used to connect (make sure the user has privilege 15): ")
    ssh_password = getpass.getpass("Please inform the password to be used to connect: ")
    enable_secret = getpass.getpass("ASA in version 8.2 and below uses enable secret, please inform them now: ")
    print("------------------------------------------------------------------------")
elif asaversion == '8.3':
    print(" ")
    ...
    hostname = input("Please inform the IP or hostname to connect: ")
    ssh_username = input("Please inform the username to be used to connect (make sure the user has privilege 15): ")
    ssh_password = getpass.getpass("Please inform the password to be used to connect and enable: ")
...
def isOpen(ip,port):
    try:
        ...
        print('Verifying if host is listening on remote port ' + str(remoteport) + ' ...')
        s.connect((ip, port))
        s.shutdown(2)
        print(" ")
        print('Host is listening on remote port ' + str(remoteport) + '... proceeding')
        print(" ")
    except socket.error:
        print(" ")
        print('Host its not listening on indicated remote port ' + str(remoteport) + ', program will exit now')
        sys.exit()
...
if version_running <= '8.2':
    if version_running <= '8.2':
        ...
        print("Informed version " + asaversion + " and device version " + version_running + " are compatible, proceeding...")
    else:
        print("You informed version " + asaversion + " when program started...") 
        print("Informed version " + asaversion + " and device version " + version_running + " are not compatible to configure")
...
company02 = input("Enter abbreviated name of partner company where the connection will be made: ")
print("object-group network " + company01 + "-" + company02, file=open("/tmp/temp_network-companies.txt", "w"))
print("description Hosts/Networks to protect in " + company01 + " side", file=open("/tmp/temp_network-companies.txt", "a")) 
print(" ")
...
print("object-group network " + company02 + "-" + company01, file=open("/tmp/temp_network-companies.txt", "a"))
print("description Hosts/Networks to protect in " + company02 + " side", file=open("/tmp/temp_network-companies.txt", "a")) 
print(" ")
...
# Define dictionaries to read actual configuration and determine the index of the new configuration
#crypto_map_properties = { 'index': ' ', 'parameter': ' ', 'property': ' ', 'value': '' }
#crypto_policy = { 'index': ' ', 'authentication': ' ', 'encryption': ' ', 'hash': ' ', 'group': ' ', 'lifetime': '' } 
